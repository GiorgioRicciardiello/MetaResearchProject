"""
Meta analysis for diagnostic test. Focus on sensitivy, specifciy and AUC
...
import pathlib
from library.stat_models import compute_Q_and_I2
...
def compute_2x2_metrics(Sens:Union[int, float],
                        Spec:Union[int, float],
                        N:int,
                        prevalence=0.5) -> Dict[str, Union[float, int]]:
    """
    Compute the 2x2 contingency table counts based on sensitivity, specificity, sample size, and prevalence.

    Parameters:
        Sens (float): Sensitivity as a percentage (e.g., 75 for 75%).
        Spec (float): Specificity as a percentage (e.g., 80 for 80%).
        N (int): Total sample size.
        prevalence (float): Prevalence of the condition in the sample (default is 0.5).

    Returns:
        tuple: A tuple (TP, FP, FN, TN)
    """
    # Calculate the number of positive and negative cases based on prevalence.
    n_pos = round(N * prevalence)
    n_neg = N - n_pos

    # Calculate the 2x2 table counts.
    TP = round((Sens / 100) * n_pos)
    FN = n_pos - TP
    TN = round((Spec / 100) * n_neg)
    FP = n_neg - TN
...
def process_study(df_metrics:pd.DataFrame, threshold:int) -> pd.DataFrame:
    """
    Structure the input dataset such that we get the sample size, sensitivity and specificity for the studies in a
    single dataframe and with the pre-processed value
    :param df_metrics:
    :param threshold:
    :return:
    """
    ...
    df.rename(columns={f'Sensitivity_AHI_{threshold}': 'sensitivity',
                       f'Specificity_AHI_{threshold}': 'specificity',
                       f'AUC_{threshold}': 'AUC'}, inplace=True)
    ...
    # df['2x2_metrics'] = df.apply(
    #     lambda row: compute_2x2_metrics(
    #         Sens=row['sensitivity'],
    #         Spec=row['specificity'],
    #         N=row['sample_size'] if row['k-fold'] is not None else row['test_data'],
    #         prevalence=0.5
    #     ),
    #     axis=1
    # )
    # We are reporting the metrics from the validation fold or test data. We do not the sample size for training
    # we assume that the fold or split has a balanced distribution of the samples
    metrics_tab = df.apply(
        lambda row: compute_2x2_metrics(
            Sens=row['sensitivity'],
            Spec=row['specificity'],
            N=row['test_data'],
    ...
    df_metrics = pd.DataFrame(metrics_tab.tolist(), index=metrics_tab.index)
    df = pd.merge(df,df_metrics, how='left', left_index=True, right_index=True)
...
def compute_forest_plot_dor(df_studies: pd.DataFrame,
                            ...
                            title:str='',
                            file_name:str='forest_plot_dor_threshold.png') -> pd.DataFrame:
    """
    ...
    #
    df['DOR'] = (df['sens_prop'] * df['spec_prop']) / ((1 - df['sens_prop']) * (1 - df['spec_prop']))
    ...
    plt.errorbar(df['DOR'],
                 y_positions,
                 xerr=[df['DOR'] - df['ci_lower'], df['ci_upper'] - df['DOR']],
                 fmt='o',
                 color='black',
                 ecolor='gray',
                 capsize=3)
...
def compute_sroc_by_threshold(df_studies: pd.DataFrame,
                              output_path: Optional[pathlib.Path] = None,
                              title: str = 'sROC Curve',
                              file_name: str = 'forest_plot_dor_threshold.png'
                              ) -> pd.DataFrame:
    """
    ...
    This function implements a Moses-Littenberg approach:
      1. Computes the false positive rate (FPR = 1 - specificity).
      2. Applies the logit transformation to both sensitivity (TPR) and FPR.
      3. Calculates D = logit(TPR) - logit(FPR) and S = logit(TPR) + logit(FPR) for each study.
      4. Performs linear regression: D = a + b * S.
      5. Generates the sROC curve using:
             TPR = expit(a + b * log(FPR/(1 - FPR)))
         over a range of FPR values.
      6. Computes an approximate AUC using AUC = Φ(a / sqrt(1 + b²)).
    ...
    # TPR = expit(a + b * log(FPR/(1-FPR)))
    fpr_values = np.linspace(eps, 1 - eps,
                             num=100)
...
if __name__ == "__main__":
    ...
    col_static = ['study_id',
                  # 'study_name',
                  'sample_size', 'train_data', 'test_data']
    ...
    for thr in thresholds:
        ...
        df_result = meta_analysis_by_threshold(df_study=df_study,
                                               output_path=None)
        compute_forest_plot_dor(df_studies=df_study,
                                output_path=None,
                                title=f'Forest Plot of Diagnostic Odds Ratios for AHI Threshold {thr}',
                                file_name=f'forest_plot_dor_threshold_{thr}.png')

        compute_sroc_by_threshold(df_studies=df_study,
                                output_path=None,
                                title=f'sROC Curve for AHI Threshold {thr}',
                                file_name=f'sroc_curve_threshold_{thr}.png')
